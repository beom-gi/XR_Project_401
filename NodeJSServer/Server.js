...
wss.on('connection', function connection(ws) {
    ...
    ws.on('message', (data) => {                        //Message 이벤트
        const jsonData = JSON.parse(data);              //받은 데이터를 json파싱한다.
        ...
        if(requestType == 0)
        {
            wss.clients.forEach((client) => {
                client.send(data);                          //접속한 클아이언트들에게 send 한다.
            });
        }
    });
    ...
    ws.send(JSON.stringify({clientID: ws.clientID}));   //클라이언트에게 임시 유저이름 전송
...
function joinRoom(params, ws) {
    const room = params;
    if(!Object.keys(rooms).includes(rooms))
    {
        console.warn(`Room ${room} does net exist !`);       //룸이 없으면 존재하지 않는다는 메세지
    }
    if(rooms[room].length >= maxClients)                    //5명 이상 못 들어가게 막는다.
    {
        console.warn(`Room ${room} is full !`)              //룸이 없으면 존재하지 않는다는 메세지
    }

    rooms[room].push(ws);                                   //ws 소켓을 룸에 넣는다.
    ws["room"] = room;
...
function leaveRoom(params) {          //룸을 나갈경우
    ...
    if(rooms[room].length > 0)          //룸이 존재할 때
    {
        ...
        if(rooms[room].length == 0)         //룸이 0명이 되었을때
        {
            close(room);
        }
    }
